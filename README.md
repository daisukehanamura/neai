...
画面右上の「設定」から、**端末上で**切り替えられる。`.env` の編集も再起動も要らない。
設定は localStorage に保存される。

| 項目 | 反映 |
|---|---|
| ウェイクワード（プリセット選択） | 待機し直すと反映 |
| コンプレッサ / ハイパス / ノイズゲート | 待機し直すと反映 |
| 入力ゲイン | 待機し直すと反映 |
| モデル（通常 / mini） | 次の接続から |
| 無操作で切断する秒数 | 次の接続から |
| 回答を画面に残す秒数 | すぐ |

ウェイクワードの候補は**すべて日本語モデルの語彙に存在することを確認済み**。
認識しやすさは「長い・子音が立っている・日常会話に出てこない」で決まる。

| 候補 | 評価 |
|---|---|
| **ねえクラピカ**（既定） | 破裂音が3つ。日常語と衝突しない |
| ハンターハンター | 8モーラと長く、判断材料が多い |
| 幻影旅団 / キメラアント | 長く、まず言わない語 |
| ねえジャービス | 濁音と破裂音が立つ |
| ねえAI | 母音中心で弱い。テレビ等で誤検出しやすい |

モデルを mini にすると料金は約3分の1になる。Worker 側は許可リストで検証しており、
端末から任意のモデル名を通せるわけではない。
