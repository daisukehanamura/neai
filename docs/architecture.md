...
3分間つなぎっぱなしで約10円かかる（30分なら約85円）。

ただし**決まった言い回しならローカルコマンド層が引き取り、呼び出しも無料になる**（後述）。
詳細は [cost.md](cost.md)。

### ローカルコマンド層

Vosk はウェイクワードのために常時動いている**全文認識器**なので、
その結果をそのままコマンド照合に使える。追加コストはゼロ。

```
発話 → Vosk（端末内）
        ├─ コマンドに一致 → 端末内で実行 → 端末内の音声合成で返事
        │                    OpenAI に繋がない。課金ゼロ
        │
        └─ 一致しない ＋ ウェイクワード → WebRTC 接続 → AI が応答
```

対象は「言い回しが決まっていて、AI に考えさせる必要がないもの」だけ。
タイマーの設定・取消、残り時間、時刻。曖昧なものは AI に任せる。

**「ねえクラピカ、タイマー三分」のように続けて言われた場合を考慮して、
ウェイクワードを聞いてから会話を開くまで 700ms 待つ。**
その間にコマンドが確定したら会話を開かない。
...
│  ☀ 23° 市川市    │   ⇄   │ これはコーヒー │
│  今 月 火 水 木 金 土 │        │ カップです。   │
│  26 29 30 31 31 26 23│        │              │
│  ▂ ▁ ▂ ▂ ▄ ▅ ▆  │        │ ⏱ 3分  2:14   │
└────────────────┘        └──────────────┘
```

週間予報は**気温を数字、降水確率を棒の高さ**で表す。色で塗り分けないのは、
離れて見ると色の差が効かないため。数字は50%以上の日だけに付ける。

**待機中に何も出ていないのは、常設端末として画面を遊ばせている。**
時計・天気・動いているタイマーを出して、話しかけなくても役に立つ状態にしてある。

回答が出た後は `keepSec`（既定90秒）だけ会話画面のまま残り、それから待機画面へ戻る。
聞き逃したときに読み返せるようにするため。

開発用の情報（レイテンシ、費用、トークン内訳、ログ、カメラ切替）は
ヘッダの「ログ」で出し入れする。ふだんは隠しておく。
