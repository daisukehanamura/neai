...
│     ├ vad.ts          声の判定（Silero VAD）。声のある区間だけ認識に回す
...
中身は `.amb-clock`（時計・日付）と `.amb-info`（天気・週間予報・タイマー）の
2グループに分けてある。**縦向きでは `display: contents` で素通しになり、
横向きのときだけ左右2カラムに組み替わる。** 分岐は CSS 側だけにある。
...
**マイクは**アプリの生存期間中ずっと保持する。**カメラは既定では保持しない**
（720p の取り込みを24時間回し続けると、画面の次に電力を使う）。

- `acquire(withVideo)` … 取得済みなら同じストリームを返す。二度目の `getUserMedia` を呼ばない
- `ensureVideo()` … 撮る直前にカメラを取り直す。絵が出るまで待ってから返す
- `releaseVideo()` … カメラだけ手放す。**音声トラックには触らない**
...
映像を取り直せるのは `switchCamera()` が実証している通りで、
iOS で取り直せないのは**マイクのほう**。設定「カメラを常に掴んだままにする」で
保持する側にも切り替えられる（撮影が速くなる代わりに発熱が増える）。
...
- 音響チェーン: **ハイパス(100Hz) → コンプレッサ → ゲイン → ノイズゲート → 声の判定**
  - コンプレッサはゲインと違い「小さい音だけ持ち上げる」ので、遠くの声に効く
  - 声の判定(VAD)は `vad.ts`。声のある区間だけ Vosk に渡し、待機中の電力を下げる。
    無音を渡さなくなるぶん、発話の終わりは `retrieveFinalResult()` で自分で伝える
- 3つのモードを持つ
