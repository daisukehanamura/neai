...
**読み上げ中は WebRTC のマイクを止め、Vosk だけが端末内で聞き続ける。**
自分の声や物音で応答が中断され作り直しになるのを防ぎつつ、
「ストップ」と言えば止められる状態を保つ。

中断されないための手当ては2重にしてある。

1. `turn_detection.interrupt_response: false` … OpenAI 側で、音を拾っても
   応答を中断しない設定にする。これが根本的な手当て
2. 読み上げ中はマイクの送信を止める … そもそも音を送らない

**マイクを戻すのは「鳴り終わってから」。** `response.done` は
「作り終えた」であって「鳴り終えた」ではない。受信音のエネルギーが
0.6秒増えなくなったのを見て戻す。これを待たないと読み上げの
終わり際を自分で拾う。
